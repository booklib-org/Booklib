...
@section("breadcrumbs")
    <div class="c-subheader px-3">
        ...
        <ol class="breadcrumb border-0 m-0">
            ...
            <li class="breadcrumb-item"> {{$library->name}}</li>
...
@section("page")
    ...
    <div class="row">
        <div class="col-md-9">
            <div class="card">
                <div class="card-header">Edit Library: {{$library->name}}</div>
                <form class="form-horizontal" action="/manage/libraries/{{$library->id}}" method="post">
                    <div class="card-body">
                            @csrf
                        @method('PUT')
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <input class="form-control" id="name" type="text" name="name" placeholder="Library Name" required value="{{$library->name}}"><span class="help-block">Please enter the library name you'd like to set</span>
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <select class="form-control" id="type" name="type">
                                        <option @if($library->type == "Books") selected @endif>Books</option>
                                        <option @if($library->type == "Comics") selected @endif>Comics</option>
                                    </select>
                                ...
                                <div class="col-md-9">
                                    <ul class="list-group folderList">
                                        ...
                                        @foreach($library->folders as $folder)
                                            <li class="list-group-item d-flex list-group-item-action justify-content-between align-items-center" id="RemoveFolder" value="{{$folder->path}}" >{{$folder->path}}<button class="btn btn-danger" id="RemoveFolderButton"  type="button" value="{{$folder->path}}">Remove</button></li>
                                            <input type="hidden" name="folder[]" id="folder" value="{{$folder->path}}">
                                        @endforeach
                                    </ul>
